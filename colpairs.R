  ...
  table <-raw
    ...
    select <- t(combn(from,2))
      ...
      if (i>1){
        ...
        diff<-table[,c(left,right)]
        diff<-as.data.frame(diff[,1]-diff[,2])
        tmp<-cbind.data.frame(id,id,table[,c(1,left,right)],diff)
        tmp[,1]<-""
        names(tmp)[c(1,2,3,6)]<-c(" "," "," ","Difference")
        tmp <- tmp[order(-(tmp[,6])),]
    ...
    writeData(wb, sheet = 1, "Columnwise Group Differences", startCol = 2, startRow=2)
